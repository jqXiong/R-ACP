...
if __name__ == '__main__':
    ...
    parser.add_argument('--refine_adaptive_keep_margin', type=float, default=0.0)
    parser.add_argument('--refine_scale_min', type=float, default=0.6)
    parser.add_argument('--refine_scale_max', type=float, default=1.0)
    ...
    parser.add_argument('--refine_snr_aware', action='store_true')
    parser.add_argument('--refine_snr_reference_db', type=float, default=10.0)
    parser.add_argument('--refine_low_snr_camera_bonus', type=int, default=0)
    parser.add_argument('--refine_low_snr_channel_bonus', type=float, default=0.0)
    parser.add_argument('--refine_low_snr_drop_floor', type=float, default=0.0)
    parser.add_argument('--refine_low_snr_scale_boost', type=float, default=0.0)
    parser.add_argument('--refine_use_cross_view_fusion', action='store_true')
    parser.add_argument('--refine_use_bev_attention', action='store_true')
    parser.add_argument('--refine_strong_head_weight', type=float, default=0.0)
    ...
    parser.add_argument('--rate_aware_training', action='store_true', help='enable token-level rate-aware dropout and JSCC scaling')
    parser.add_argument('--target_comm_kb', type=float, default=28.5, help='target communication budget (KB/frame) for rate-aware proxy')
    parser.add_argument('--min_keep_per_camera', type=int, default=1, help='minimum kept tokens per camera when rate-aware dropout is enabled')
    parser.add_argument('--keep_latest_token', action='store_true', help='always keep latest token per camera in rate-aware dropout')
    parser.add_argument('--frame_dropout_noise_std', type=float, default=0.05, help='score noise std for stochastic token selection during training')
    parser.add_argument('--lambda_consistency', type=float, default=0.03, help='weight for reliability consistency regularization')
    parser.add_argument('--rate_view_dropout_prob', type=float, default=0.2, help='drop probability for weak-view consistency branch')
